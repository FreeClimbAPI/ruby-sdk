    ...
    class RequestVerifier
        class << self
            ...
            def verify_request_signature(request_body, request_header, signing_secret, tolerance=DEFAULT_TOLERANCE)
                ...
                request_verifier_object.check_request_body(request_body)
                request_verifier_object.check_request_header(request_header)
                request_verifier_object.check_signing_secret(signing_secret)
                request_verifier_object.check_tolerance(tolerance)
                info = Freeclimb::SignatureInformation.new(request_header)
                request_verifier_object.verify_tolerance(info, tolerance)
                request_verifier_object.verify_signature(info, request_body, signing_secret)
            end
        end
