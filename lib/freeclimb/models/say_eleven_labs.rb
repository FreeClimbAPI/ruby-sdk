...
module Freeclimb
  ...
  class SayElevenLabs
    ...
    # Number of times the text is said. Specifying '0' causes the `SayElevenLabs` action to loop until the Call is hung up.
    attr_accessor :loop
    ...
    def initialize(attributes = {})
      if !attributes.is_a?(Hash)
        fail ArgumentError, "The input argument (attributes) must be a hash in `Freeclimb::SayElevenLabs` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if !self.class.attribute_map.key?(k.to_sym)
          fail ArgumentError, "`#{k}` is not a valid attribute in `Freeclimb::SayElevenLabs`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [text, loop, privacy_mode, engine].hash
    end
